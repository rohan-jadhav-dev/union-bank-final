...
// Same business logic as the full-page conversation-desk.js (lead auto-fill,
// cleanModelText sanitization, smart replies, manual checklist, save-to-records
// -> lead-form.html flow) — only the UI container changed to a floating widget.
// ============================================================================
...
(function () {
  ...
  // ── LEAD AUTO-FILL FORM SCHEMAS (same as old conversation-desk.js) ────────
  ...
  // Strips stray JSON / markdown fences / preambles the backend LLM sometimes
  // returns instead of a clean string. Same logic as old conversation-desk.js.
  ...
  class ChatWidget {
    constructor() {
      ...
      this.isActive = false;
      this.lastLeadSources = {}; // key -> "backend" | "fallback" | ""
        ...
        <div class="cw-body">
          <div class="cw-chat-area">
            ...
            <div class="cw-input-bar" id="cwInputBar" style="display:none">
              <button class="cw-mic-btn" id="cwCustomerMic" title="Record customer speech">${ICONS.mic}</button>
            ...
            <div class="cw-summary-card" style="margin-bottom:16px">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;gap:8px;flex-wrap:wrap">
                ...
                <div style="display:flex;gap:6px">
                  <button class="cw-modal-btn ghost" id="cwReExtractBtn" style="flex:none;height:32px;padding:0 12px;font-size:11.5px">↻ Re-extract</button>
                  <button class="cw-modal-btn primary" id="cwSubmitLeadBtn" style="flex:none;height:32px;padding:0 12px;font-size:11.5px">Send to Bank</button>
                </div>
              </div>
              <div id="cwLeadFormBody">
    ...
    close() {
      if (this.isActive) {
        if (!confirm("Close conversation? You can reopen it from the sidebar.")) return;
      }
    ...
    async _processCustomerAudio() {
      try {
        ...
        if (data.success && cleanCustomerText) {
          if (this._isUnclearTranscript(cleanCustomerText)) {
            this._addCustomerBubble(cleanCustomerText, "(unclear — ask customer to repeat)", data.stt_engine);
            this.conversationLog.push({ role: "customer", text: cleanCustomerText, translation: "(unclear)" });
        ...
        this._hideProcessingIndicator();
        this._showToast("Backend error. Check connection.", true);
        this._setStatus("", "Ready");
      }
    }
    ...
    _manualNextStep() { this._advanceStep(); this._showStaticQuickReplies(); }
    ...
    async _updateStepFromLLM() {
      ...
      try {
        ...
        if (data.next_question) this._addSuggestionBadge(`💡 Suggested: ${cleanModelText(data.next_question)}`);
    ...
    async endSession() {
      ...
      if (this.els.submitLeadBtn) { this.els.submitLeadBtn.disabled = false; this.els.submitLeadBtn.textContent = "Send to Bank"; }

      await Promise.all([this._fetchSummary(), this.fetchLeadForm()]);
    }
    ...
    async _fetchSummary() {
      try {
        ...
        if (data.success) {
          ...
          this.els.englishSummary.textContent = "Error: " + (data.error || "");
        }
      } catch (e) { this.els.englishSummary.textContent = "Network error."; }
    }

    // ── LEAD AUTO-FILL (same logic as old conversation-desk.js) ─────────
    async fetchLeadForm() {
      const fields = LEAD_FORM_FIELDS[this.selectedProcess] || LEAD_FORM_FIELDS["General enquiry"];
      if (this.els.leadFormBody) {
      ...
      try {
        ...
        const data = await res.json();
        if (data.success && data.lead) leadData = data.lead;
        else this._showToast(data.error || "Could not auto-extract lead — fill manually", true);
      } catch (e) {
        this._showToast("Lead extraction failed — fill manually", true);
      }

      this._renderLeadForm(fields, leadData);
    }

    _renderLeadForm(fields, leadData) {
      const container = this.els.leadFormBody;
      if (!container) return;
      container.innerHTML = "";
      fields.forEach(f => {
        ...
        row.innerHTML = `
          ...
          <input type="text" data-field="${escHtml(f.key)}" value="${escHtml(value)}"
            ...
            font-size:13px;font-family:inherit;color:var(--navy);background:${value ? "rgba(184,146,61,0.06)" : "var(--white)"}"
    ...
    async submitLead() {
      ...
      if (inputs.length === 0) { this._showToast("No lead form to submit", true); return; }

      const lead = {};
      inputs.forEach(inp => { lead[inp.dataset.field] = inp.value.trim(); });

      const btn = this.els.submitLeadBtn;
    ...
    // ── SAVE TO RECORDS — same flow as old conversation-desk.js:
    // saves a local session record (for dashboard.js's Overview / Session
    // History / Bilingual Summary tabs to read from localStorage `va_sessions`),
    // then hands the conversation off to lead-form.html via sessionStorage and
    // navigates there so staff can review and click "Send to Bank".
    // ─────────────────────────────────────────────────────────────────────
    saveToRecords() {
      ...
      try {
        ...
        const sessionRecord = {
          ...
          duration: duration,
          durationMinutes: this._parseDurationMinutes(duration),
      ...
      sessionStorage.setItem("va_lead_process", this.selectedProcess);
      sessionStorage.setItem("va_lead_language", this.selectedLanguage);
      sessionStorage.setItem("va_lead_duration", duration);

      if (this.els.saveRecordBtn) { this.els.saveRecordBtn.textContent = "✓ Saved!"; this.els.saveRecordBtn.disabled = true; }
      this._showToast("Saved — opening lead form ✓");

      setTimeout(() => { window.location.href = "lead-form.html"; }, 900);
    }

    _parseDurationMinutes(duration) {
      const m = /^(\d+):(\d+)$/.exec(duration || "");
      if (!m) return 0;
      return Math.round((parseInt(m[1], 10) * 60 + parseInt(m[2], 10)) / 60 * 10) / 10;
    }
    ...
    _buildProcessPanel() { this._updateProcessPanel(); }
    ...
    _scrollChat() {
      requestAnimationFrame(() => { this.els.messages.scrollTop = this.els.messages.scrollHeight; });
    }
